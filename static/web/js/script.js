...
(function($) {
	...
	// add .row class to .form-horizontal and .col-md-3 to add its children with .mb-3 class
	$(".form-horizontal-additional-fee #div_id_fee_name").hide();
	$(".form-horizontal-additional-fee").each(function () {
		$(this).addClass("row");
		$(this).find(".mb-3").addClass("col-lg-4 col-md-4 col-sm-6 col-12");
	});
	

	$(".form-horizontal-additional-fee #id_fee").change(function () {
        var isTrue = $(this).val() === "other";
        
        // Show/hide the elements based on the selected value
        $(".form-horizontal-additional-fee #div_id_fee_name").toggle(isTrue);
    
        // Set the 'required' property and append/remove the asterisk accordingly
        $(".form-horizontal-additional-fee #id_fee_name").prop('required', isTrue).siblings("label").find(".asteriskField").remove();
        if (isTrue) {
            $(".form-horizontal-additional-fee #id_fee_name").siblings("label").append('<span class="asteriskField">*</span>');
            var inputsAndSelects = $('.parent-detail-container').find('input, select');
            inputsAndSelects.val(null);
        }
	...
	if ($(".dateinput").length > 0) {
	  ...
	  if ($(".timeinput").length > 0) {
		$(".timeinput").datetimepicker({
			format: 'LT', // 'LT' stands for 'h:mm A', which represents time format
			pickDate: false, // Disable date picking
			pickSeconds: false // Disable seconds picking
		});
	}
